...
- Memory/CLI: add `openclaw memory trim` to show MEMORY.md size vs the bootstrap limit and open it in `$EDITOR` for pruning; also surface a per-file size warning in `openclaw memory status` when MEMORY.md exceeds the bootstrap char limit so over-limit workspaces are immediately visible.
